...
try {
    ...
    // Thêm donhang - trạng thái: Đã thanh toán
    // Lưu ý: hệ thống của bạn đã có TRIGGER tự động giảm tồn kho/ghi sử dụng voucher khi INSERT/UPDATE sang trạng thái đã thanh toán,
    // nên ở đây chúng ta chỉ lưu đơn với trạng thái 'Đã thanh toán' và không thao tác giảm tồn kho trực tiếp trong PHP.
    if ($voucher_id > 0) {
        $sqlInsert = "INSERT INTO donhang (madonhang, iduser, diachi, sodienthoai, soluong, tongtien, ngaydat, ngaygiaodukien, trangthai, voucher_id, ma_voucher, gia_tri_giam, tong_truoc_giam)
            VALUES ('{$madonhang}', {$iduser}, '{$diachi}', '{$sodienthoai}', {$soluong}, '{$tongtien}', NOW(), '{$ngaygiaodukien}', 'Đã thanh toán', {$voucher_id}, '{$ma_voucher}', {$gia_tri_giam}, {$tong_truoc_giam})";
    } else {
        $sqlInsert = "INSERT INTO donhang (madonhang, iduser, diachi, sodienthoai, soluong, tongtien, ngaydat, ngaygiaodukien, trangthai)
            VALUES ('{$madonhang}', {$iduser}, '{$diachi}', '{$sodienthoai}', {$soluong}, '{$tongtien}', NOW(), '{$ngaygiaodukien}', 'Đã thanh toán')";
    }
    ...
    // Thêm chi tiết đơn hàng (không giảm tồn kho ở đây - trigger DB sẽ xử lý việc giảm tồn kho nếu đã cấu hình)
    foreach ($cartItems as $item) {
        $idsp = intval($item['idsp']);
        $sl = intval($item['soluong']);
