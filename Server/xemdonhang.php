...
// Kiểm tra biến iduser
if (!isset($_POST['iduser']) || trim($_POST['iduser']) === '') {
    $arr = [
        'success' => false,
...
if ($isadmin === 1) {
    ...
    if ($stmt = mysqli_prepare($conn, $checkSql)) {
        ...
        // Lấy tất cả đơn hàng của user có role = 0 hoặc role IS NULL (treat NULL as regular user)
...
$detailSql = "SELECT ct.iddonhang AS iddonhang, ct.idsp AS idsp, ct.soluong AS soluong, ct.gia AS gia, sp.tensp AS tensp, sp.hinhanh AS hinhanh
